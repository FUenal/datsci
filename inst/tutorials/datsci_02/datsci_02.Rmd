...
Welcome to the *Data Science: Data Visualization* (`datsci_02`) module. It is designed to teach you data visualization techniques to communicate data-driven findings.
...
- The *cumulative distribution function (CDF)* is a function that reports the proportion of data below a value 𝑎 for all values of a: $F(a)=\mbox{Pr}(x \leq a)$.

- The proportion of observations between any two values 𝑎 and 𝑏 can be computed from the CDF as $F(b) - F(a)$.
...
With the right argument, `ggplot` automatically shades the intersecting region with a different color. We will show examples of __ggplot2__ code for densities in the Book [(Section - 9  Data visualization in practice) ](https://rafalab.github.io/dsbook/gapminder.html#gapminder) as well as [(Section - 8.16 ggplot2 geometries) ](https://rafalab.github.io/dsbook/distributions.html#other-geometries).
...
The pre-built functions `mean` and `sd` (note that for reasons explained in the textbook [(Section - 16.2 Data-driven models) ](https://rafalab.github.io/dsbook/models.html#data-driven-model), `sd` divides by `length(x)-1` rather than `length(x)`) can be used here:
```{r}
m <- mean(x)
s <- sd(x)
c(average = m, sd = s)
...
The above code is included to help describe QQ-plots. However, in practice it is easier to use the **ggplot2** code described in the Book [(Section - 8.16 ggplot2 geometries) ](https://rafalab.github.io/dsbook/distributions.html#other-geometries): 
...
We discuss how to make boxplots in the Book [(Section - 8.16 ggplot2 geometries)](https://rafalab.github.io/dsbook/distributions.html#other-geometries).
...
This is preferred as it makes it easier to read the text. In the Book [(Section - 7.11 Add-on packages)](https://rafalab.github.io/dsbook/ggplot2.html#add-on-packages) we learn a better way of assuring we can see the points and the labels.
...
For qq-plots we use the `geom_qq` geometry. From the help file, we learn that we need to specify the `sample` (we will learn about samples in a later section). Here is the qqplot for men heights.
...
we will receive an error: `Error: expecting result of length one, got : 2`. With the function `summarize`, we can only call functions that return a single value. In the Book [(Section - 4.12 do)](https://rafalab.github.io/dsbook/tidyverse.html#do), we will learn how to deal with functions that return more than one value.
...
We remake the plots using only countries with data available for both years. In the data wrangling part of this course, we will learn __tidyverse__ tools that permit us to  write efficient code for this, but here we can use simple code using the `intersect` function:
...
```{r barplot-better-than-area, out.width="80%", echo=FALSE}
gdp_data %>% 
  filter(y == "Area") %>%
