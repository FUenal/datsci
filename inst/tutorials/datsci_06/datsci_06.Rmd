...
Welcome to the *Data Science: Wrangling* (`datsci_06`) module. It is designed to teach you data wrangling, the process of converting raw data to the formats needed for further analysis.
...
Using a combination of guided introduction lectures and more independent in-depth exploration, you will get to practice your new `r rproj()` skills on real-life applications.
...
- Importing data into `r rproj()` from different file formats 
...
**Textbook link** The content is discussed within the [textbook section - 20 Introduction to data wrangling](https://rafalab.github.io/dsbook/introduction-to-data-wrangling.html).
  
</div>


The datasets used in this course have been made available to you as R objects, specifically as data frames. The US murders data, the reported heights data, and the Gapminder data were all data frames. These datasets come included in the __dslabs__ package and we loaded them using the `data` function. Furthermore, we have made the data available in what is referred to as `tidy` form. The tidyverse packages and functions assume that the data is `tidy` and this assumption is a big part of the reason these packages work so well together. 
...
**Textbook link** The content is discussed within the [textbook section - 5 Importing data](https://rafalab.github.io/dsbook/importing-data.html).
...
```{r, echo=FALSE}
knitr::include_graphics("./images/csv-file.png")
```
...
**Textbook link** The content is discussed within the [textbook section - 5.1 Paths and the working directory](https://rafalab.github.io/dsbook/importing-data.html#paths-and-the-working-directory).
...
The data is imported and stored in `dat`. The rest of this section defines some important concepts and provides an overview of how we write code that tells R how to find the files we want to import. The textbook [(Section - 38 Organizing with Unix)](https://rafalab.github.io/dsbook/unix.html) provides more details on this topic. 
...
**Textbook link** The content is discussed within the [textbook section - 5.2 The readr and readxl packages](https://rafalab.github.io/dsbook/importing-data.html#the-readr-and-readxl-packages).
...
**Textbook link** The content is discussed within the [textbook section - 5.5 R-base importing functions](https://rafalab.github.io/dsbook/importing-data.html#r-base-importing-functions). 
...
**Textbook link** The content is discussed within the [textbook section - 5.4 Downloading files](https://rafalab.github.io/dsbook/importing-data.html#downloading-files).
...
We will learn how to implement the tidyverse approach throughout the course, but before delving into the details, in this section we introduce some of the most widely used tidyverse functionality, starting with the __dplyr__ package for manipulating data frames and the __purrr__ package for working with functions. Note that the tidyverse also includes a graphing package, __ggplot2__, which we introduce later in the textbook [(Section - 7 ggplot2)](https://rafalab.github.io/dsbook/ggplot2.html) in the Data Visualization part of the textbook; the __readr__ package discussed in the textbook [(Section - 5 Importing data)](https://rafalab.github.io/dsbook/importing-data.html); and many others. In this section, we first introduce the concept of _tidy data_ and then demonstrate how we use the tidyverse to work with data frames in this format. 
...
**Textbook link** The content is discussed within the [textbook section -  21. Reshaping data](https://rafalab.github.io/dsbook/reshaping-data.html), the [textbook section - 21.1 gather](https://rafalab.github.io/dsbook/reshaping-data.html#gather), and the [textbook section - 21.2 spread](https://rafalab.github.io/dsbook/reshaping-data.html#spread).
  
</div>


As we have seen through the course, having data in _tidy_ format is what makes the tidyverse flow. After the first step in the data analysis process, importing data, a common next step is to reshape the data into a form that facilitates the rest of the analysis. The __tidyr__ package includes several functions that are useful for tidying data. 

We will use the fertility wide format dataset described in the textbook [(Section - 4.1 Tidy data)](https://rafalab.github.io/dsbook/tidyverse.html#tidy-data) as an example in this section. 
...
```{r, echo=FALSE}
knitr::include_graphics("./images/gather-spread.png")
```
...
**Textbook link** The content is discussed within the [textbook section -  21.3 separate](https://rafalab.github.io/dsbook/reshaping-data.html#separate), and the [textbook section - 21.4 unite](https://rafalab.github.io/dsbook/reshaping-data.html#unite). 
...
**Textbook link** The content is discussed within the [textbook section -  22 Joining tables](https://rafalab.github.io/dsbook/joining-tables.html). 
...
**Textbook link** The content is discussed within the [textbook section -  22.2 Binding](https://rafalab.github.io/dsbook/joining-tables.html#binding). 
...
**Textbook link** The content is discussed within the [textbook section -  22.3 Set operators](https://rafalab.github.io/dsbook/joining-tables.html#set-operators). 
...
**Textbook link** The content is discussed within the [textbook section - 23 Web scraping](https://rafalab.github.io/dsbook/web-scraping.html) through [textbook section - 23.2 The rvest package](https://rafalab.github.io/dsbook/web-scraping.html#the-rvest-package). 
...
**Textbook link** This page correnponds to the [textbook section - 23.3 CSS selectors](https://rafalab.github.io/dsbook/web-scraping.html#css-selectors)
...
vignette]/https://cran.r-project.org/web/packages/rvest/vignettes/selectorgadget.html) and other [tutorials based on the vignette](https://stat4701.github.io/edav/2015/04/02/rvest_tutorial/) and [here](https://www.analyticsvidhya.com/blog/2017/03/beginners-guide-on-web-scraping-in-r-using-rvest-with-hands-on-knowledge/).
...
**Textbook link** The content is discussed within the [textbook section - 24 String processing](https://rafalab.github.io/dsbook/string-processing.html). 
...
Most of the examples will come from the second case study which deals with self-reported heights by students and most of the section is dedicated to learning regular expressions (regex), and functions in the __stringr__ package.
...
**Textbook link** The content is discussed within the [textbook section - 24.4 How to escape when defining strings](https://rafalab.github.io/dsbook/string-processing.html#how-to-escape-when-defining-strings). 
...
**Textbook link** The content is discussed within the [textbook section - 24.1 The stringr package](https://rafalab.github.io/dsbook/string-processing.html#stringr). 
...
Finally, note that in this table we mention _groups_. These will be explained in the textbook [(Section - 24.5.9 Groups)](https://rafalab.github.io/dsbook/string-processing.html#groups).
...
**Textbook link** The content is discussed within the [textbook section - 24.2 Case study 1: US murders data](https://rafalab.github.io/dsbook/string-processing.html#case-study-1-us-murders-data). 
...
**Textbook link** The content is discussed within the [textbook section - 24.3 Case study 2: self-reported heights](https://rafalab.github.io/dsbook/string-processing.html#case-study-2-self-reported-heights). 
...
**Textbook link** The content is discussed within the [textbook section - 24.5 Regular expressions](https://rafalab.github.io/dsbook/string-processing.html#regular-expressions) through [textbook section - 24.5.2 Special characters](https://rafalab.github.io/dsbook/string-processing.html#special-characters). 
  
</div>


A regular expression (regex) is a way to describe specific patterns of characters of text. They can be used to determine if a given string matches the pattern. A set of rules has been defined to do this efficiently and precisely and here we show some examples. We can learn more about these rules by reading a [detailed tutorials](https://www.regular-expressions.info/tutorial.html) [or here](http://r4ds.had.co.nz/strings.html#matching-patterns-with-regular-expressions). This [RStudio cheat sheet](https://www.rstudio.com/wp-content/uploads/2016/09/RegExCheatsheet.pdf) is also very useful.
...
```
```{r, echo=FALSE}
knitr::include_graphics("./images/str_view-1.png")
```
...
```
```{r, echo=FALSE}
knitr::include_graphics("./images/str_view-2.png")
```
...
**Textbook link** The content is discussed within the [textbook section - 24.5.3 Character classes](https://rafalab.github.io/dsbook/string-processing.html#character-classes), and [textbook section - 24.5.4 Anchors](https://rafalab.github.io/dsbook/string-processing.html#anchors), and [textbook section - 24.5.5 Quantifiers](https://rafalab.github.io/dsbook/string-processing.html#quantifiers). 
...
```
```{r, echo=FALSE}
knitr::include_graphics("./images/str_view-3.png")
```
...
no <- c("12", "123", " 1", "a4", "b")
s <- c(yes, no)
knitr::include_graphics("./images/str_view-4.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("./images/str_view-5.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("./images/str_view-6.png")
```
...
**Textbook link** The content is discussed within the [textbook section - 24.5.9 Groups](https://rafalab.github.io/dsbook/string-processing.html#groups)
...
**Textbook link** The content corresponds to the [textbook section - 24.7 Testing and improving](https://rafalab.github.io/dsbook/string-processing.html#testing-and-improving)
...
**Textbook link** The content corresponds to the [textbook section - 24.10.1 The extract function](https://rafalab.github.io/dsbook/string-processing.html#the-extract-function)
...
In the textbook [(Section - 21.3 separate)](https://rafalab.github.io/dsbook/reshaping-data.html#separate) we learned about the `separate` function, which can be used to achieve our current goal:
...
**Textbook link** This page corresponds to material in  [textbook sections - 24.7 through 24.10 ](https://rafalab.github.io/dsbook/string-processing.html#testing-and-improving)
...
**Textbook link** This section corresponds to the [textbook section - 24.11 String splitting](https://rafalab.github.io/dsbook/string-processing.html#string-splitting)
...
**Textbook link** This section corresponds to the [textbook section - 24.12 Case study 3: extracting tables from a PDF](https://rafalab.github.io/dsbook/string-processing.html#case-study-3-extracting-tables-from-a-pdf)
...
**Textbook link** This content corresponds to the [textbook section - 24.13 Recoding](https://rafalab.github.io/dsbook/string-processing.html#recode)
...
**Textbook link** This content corresponds to the [textbook section - 25 Parsing dates and times](https://rafalab.github.io/dsbook/parsing-dates-and-times.html)
...
**Textbook link** This page corresponds to the [textbook section - Chapter 26 Text mining](https://rafalab.github.io/dsbook/text-mining.html)
...
For each word, we want to know if it is more likely to come from an Android tweet or an iPhone tweet. In the textbook [(Section - 15.10 Association tests)](https://rafalab.github.io/dsbook/inference.html#association-tests) we introduced the odds ratio as a summary statistic useful for quantifying these differences. For each device and a given word, let's call it `y`, we compute the odds or the ratio between the proportion of words that are `y` and not `y` and compute the ratio of those odds. Here we will have many proportions that are 0, so we use the 0.5 correction described in the textbook [(Section - 15.10 Association tests)](https://rafalab.github.io/dsbook/inference.html#association-tests).
